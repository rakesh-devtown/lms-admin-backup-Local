  ...
  reducers: {
    ...
    setCurrentStudentEnrollment: (state, action) => {
      try {
        ...
        const findIndexOfStudent = student.enrollments.findIndex(
        ...
        if (findIndexOfStudent !== -1) {
          ...
          const newEnrollment = {
            ...student.enrollments[findIndexOfStudent],
            isArchived: archived,
          };
          const enrollmentWithoutIndex = student.enrollments.filter(
            (student) => student.id !== enrollmentId,
          );
    ...
    setSectionItemData: (state, action) => {
      ...
      const findIndexOfSection = state.currentCourse?.sections.findIndex(
        (section) => section.id === sectionId,
      );
      if (findIndexOfSection !== -1) {
        const findIndexOfSectionItem = state.currentCourse?.sections[
          findIndexOfSection
        ].sectionItems.findIndex((item) => item.id === sectionItemId);
        if (findIndexOfSectionItem !== -1) {
          state.currentCourse.sections[findIndexOfSection].sectionItems[
            findIndexOfSectionItem
          ] = sectionItem;
        }
      } else {
        let idOfParentSection = null;
        state.currentCourse.sections.forEach((section) => {
          const findIndexOfSubSection = section.subsections.findIndex(
            (subSection) => subSection.id === sectionId,
          );
          if (findIndexOfSubSection !== -1) {
            const findIndexOfSectionItem = section.subsections[
              findIndexOfSubSection
            ].sectionItems.findIndex((item) => item.id === sectionItemId);
            if (findIndexOfSectionItem !== -1) {
              section.subsections[findIndexOfSubSection].sectionItems[
                findIndexOfSectionItem
              ] = sectionItem;
    ...
    deleteSectionItemFromSection: (state, action) => {
      ...
      sections.forEach((section) => {
        const findIndexOfSectionItem = section?.sectionItems.findIndex(
          (item) => item.id === sectionItemId,
        );
        if (findIndexOfSectionItem !== -1) {
        ...
        section?.subsections.forEach((subSection) => {
          const findIndexOfSectionItem = subSection?.sectionItems.findIndex(
            (item) => item.id === sectionItemId,
          );
          if (findIndexOfSectionItem !== -1) {
    ...
    pushSectionItemInSection: (state, action) => {
      ...
      if (findIndexOfSection !== -1) {
        state.currentCourse?.sections[findIndexOfSection].sectionItems.push(
          sectionItem,
        );
        ...
        state.currentCourse?.sections.forEach((section) => {
          ...
          if (findIndexOfSubSection !== -1) {
            section?.subsections[findIndexOfSubSection].sectionItems.push(
              sectionItem,
            );
            idOfParentSection = section.id;
            const totalSubSections = section?.subsections.length;
            for (
              let i = findIndexOfSubSection + 1;
              i < totalSubSections;
              i += 1
            ) {
              //state.currentCourse?.sections[i].orderNumber = state.currentCourse?.sections[i].orderNumber + 1;
        ...
        if (idOfParentSection) {
          const findIndexOfSection = state.currentCourse?.sections.findIndex(
            (section) => section.id === idOfParentSection,
          );
          if (findIndexOfSection !== -1) {
            const totalSections = state.currentCourse?.sections.length;
            for (let i = findIndexOfSection + 1; i < totalSections; i += 1) {
              //state.currentCourse?.sections[i].orderNumber = state.currentCourse?.sections[i].orderNumber + 1;
              state.currentCourse?.sections[i].subsections.forEach(
                (subSection) => {
...
export const {
  ...
  setViewCourseNull,
  setSectionItemData,
  deleteSectionItemFromSection,
  setLoading,
  setCourses,
  pushSectionItemInSection,
  addCourse,
  setViewCourse,
  updateCourseState,
...
export const getCourses = (page, limit, search) => async (dispatch) => {
  try {
    ...
    const res = await serviceGet(
      `admin/admin/v1/course?all=true&offset=${page}&limit=${limit}&search=${search || ""}`,
    );
    const { message, success, data } = res;
    if (success) {
...
export const updateCourse = (course, courseId) => async (dispatch) => {
  try {
    ...
    const res = await servicePost(
      `admin/admin/v1/course/edit?id=${courseId}`,
      course,
    );
    const { message, success, data } = res;
    if (success) {
...
export const deleteSection = (sectionId, courseId) => async (dispatch) => {
  try {
    ...
    const res = await serviceDelete(
      `admin/admin/v1/curriculum/section?id=${sectionId}`,
    );
...
export const createSubSectionOfSection = (subSection) => async (dispatch) => {
  try {
    ...
    const res = await servicePost(
      `admin/admin/v1/curriculum/subsection`,
      subSection,
    );
    const { message, success, data } = res;
    if (success) {
...
export const addSectionItem = (sectionItem) => async (dispatch, getState) => {
  try {
    ...
    if (!sectionItem?.sectionId)
      return notification.error({
        message: "Section Item Creation Failed",
        description: "Section Id is required",
      });
    const res = await servicePost(
      `admin/admin/v1/curriculum/section-item`,
      sectionItem,
    );
    const { message, success, data } = res;
    if (success) {
...
export const deleteSectionItems =
  (sectionItemId) => async (dispatch, getState) => {
    try {
      dispatch(setLoading(true));
      ...
      if (!sectionItemId)
        return notification.error({
          message: "Section Item Deletion Failed",
          description: "Section Item Id is required",
        });
      const res = await serviceDelete(
        `admin/admin/v1/curriculum/section-item?id=${sectionItemId}`,
      );
      const { message, success } = res;
      if (success) {
        notification.success({
...
export const getSectionItemById = (sectionItemId) => async (dispatch) => {
  try {
    ...
    if (success) {
      ...
      notification.error({
        message: "Section Item Fetch Failed",
        description: message,
      });
    }
  } catch (error) {
...
export const addSectionItemData =
  (sectionItem, sectionItemId) => async (dispatch, getState) => {
    try {
      dispatch(setLoading(true));
      const state = getState();
      const { currentCourse } = state.course;
      if (!sectionItemId)
        return notification.error({
          message: "Section Item Update Failed",
          description: "Section Item Id is required",
        });
      const res = await servicePost(
        `admin/admin/v1/curriculum/section-item/edit?id=${sectionItemId}`,
        sectionItem,
      );
      const { message, success, data } = res;
      if (success) {
...
export const editSection = (section) => async (dispatch, getState) => {
  try {
    ...
    const { currentCourse } = state.course;
    const res = await servicePost(
      `admin/admin/v1/curriculum/section/edit?id=${section.id}`,
      section,
    );
    const { message, success } = res;
    if (success) {
...
export const addStudentToBatch = (students) => async (dispatch) => {
  try {
    ...
    notification.error({
      message: "Student Addition Failed",
      description: data?.data?.message || error.message,
    });
  } finally {
    dispatch(setLoading(false));
  }
...
export const getBatchEnrolledStudents =
  (batchId, page, limit, search) => async (dispatch) => {
    try {
      dispatch(setLoading(true));
      const res = await serviceGet(
        `admin/admin/v1/student/batch/enrollments?id=${batchId}&page=${page}&limit=${limit}&search=${search || ""}`,
      );
      const { message, success, data } = res;
      if (success) {
...
export const getAllEnrolledStudents =
  (page, limit, search) => async (dispatch) => {
    try {
      dispatch(setLoading(true));
      const res = await serviceGet(
        `admin/admin/v1/student/student?all=true&limit=${limit}&page=${page}&search=${search || ""}`,
      );
      const { message, success, data } = res;
      if (success) {
...
export const archiveStudent = (id, archived) => async (dispatch) => {
  try {
    dispatch(setLoading(true));
    const res = await servicePut(
      `admin/admin/v1/batch/archive/${id}`,
      archived,
    );
    const { message, success, data } = res;
    if (success) {
...
export const getAllCertificatesOfCourse = (courseId) => async (dispatch) => {
  try {
    dispatch(setLoading(true));
    const res = await serviceGet(
      `admin/admin/v1/certificate?courseId=${courseId}`,
    );
    const { message, success, data } = res;
    if (success) {
...
export const createCertificateTemplate = (certificate) => async (dispatch) => {
  try {
    dispatch(setLoading(true));
    const res = await servicePost(
      `admin/admin/v1/certificate/create`,
      certificate,
    );
    const { message, success, data } = res;
    if (success) {
...
export const deleteCertificateTemplate =
  (certificateId, courseId) => async (dispatch) => {
    try {
      dispatch(setLoading(true));
      const res = await servicePost(
        `admin/admin/v1/certificate/delete?certificateId=${certificateId}`,
      );
      const { message, success, data } = res;
      if (success) {
        notification.success({
